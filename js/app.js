...
let db = JSON.parse(localStorage.getItem('freddy_db_v11')) || [];
    ...
    function parseEntryDate(entry) {
        ...
        // Soporta fechas ISO y fechas locales generadas por la app:
        // ES: 22/05/2026, 14:30:00 | EN: 05/22/2026, 14:30:00
        ...
        const normal = raw.match(/(\d{1,2})[\/.-](\d{1,2})[\/.-](\d{4})(?:,?\s*(\d{1,2}):(\d{2})(?::(\d{2}))?)?/);
        if (normal) {
            const first = Number(normal[1]);
            const second = Number(normal[2]);
            const year = Number(normal[3]);
            const hour = Number(normal[4] || 0);
            const minute = Number(normal[5] || 0);
            const secondValue = Number(normal[6] || 0);

            const dayFirstDate = new Date(year, second - 1, first, hour, minute, secondValue);
            const monthFirstDate = new Date(year, first - 1, second, hour, minute, secondValue);
            const storedMonth = entry.monthKey;

            if (storedMonth && getMonthKeyFromDate(dayFirstDate) === storedMonth) {
                return dayFirstDate;
            }

            if (storedMonth && getMonthKeyFromDate(monthFirstDate) === storedMonth) {
                return monthFirstDate;
            }

            // Si el primer número es mayor a 12, necesariamente es día.
            // Si el segundo número es mayor a 12, necesariamente es día.
            // Si ambos pueden ser mes/día, se respeta el idioma actual de la app.
            const dayFirst = first > 12 || (second <= 12 && currentLang === 'es');
            return dayFirst ? dayFirstDate : monthFirstDate;
        }
    ...
    function sortRecordsForExcel(records) {
        return records
            ...
            .sort((a, b) => {
                const dateA = parseEntryDate(a.item).getTime();
                const dateB = parseEntryDate(b.item).getTime();

                if (dateA !== dateB) {
                    return dateA - dateB;
                }

                const clientCompare = String(a.item.ter || '').localeCompare(
                    String(b.item.ter || ''),
    ...
    function exportarExcelFinal() {
        ...
        // Exporta únicamente los movimientos del mes seleccionado.
        ...
        const resumen = [
            ["FINANZAS JL - " + t.reportTitle],
            [],
            [currentLang === 'es' ? "Mes del reporte" : "Report month", excelPrintDateLabel],
            [t.workingCurrency, conf.name],
            [],
            [t.in, totalIngresos],
            [t.out, totalGastos],
            [t.deu, totalDeudas],
        ...
        const orderedMonthRecords = sortRecordsForExcel(selectedMonthRecords);

        orderedMonthRecords.forEach((item, index) => {
            let tipoLegible = item.type;
        ...
        resumen.push([]);
        resumen.push([`© 2026 FINANZAS JL · ${t.autoReport}`]);
        ...
        // Activa filtros en la tabla de movimientos para que el Excel quede más fácil de revisar.
        worksheet["!autofilter"] = {
            ref: `A12:H${12 + orderedMonthRecords.length}`
        };
        ...
        const fileName = `FINANZAS_JL_Reporte_${conf.name}_${excelPrintDate}.xlsx`;
